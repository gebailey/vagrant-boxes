...
# General settings
vboxmanage modifyvm mate --description "Fedora 41 with MATE Desktop"
...
vboxmanage storageattach mate --storagectl IDE --port 1 --device 0 --type dvddrive --medium /iso/Fedora-Server-netinst-x86_64-41-1.4.iso
...
vboxmanage controlvm mate keyboardputstring " selinux=0 inst.selinux=0 inst.text inst.ks=https://lxpro.com/fedora41-mate.ks"
# Ctrl-X
...
vboxmanage export mate -o fedora41-mate-$(date +%Y%m%d).ova --vsys 0 --product fedora41-mate --version $(date +%Y%m%d) --description "Fedora 41 with MATE Desktop"
