...
# General settings
vboxmanage modifyvm mate --description "AlmaLinux 9 Developer Workstation with MATE Desktop"
...
vboxmanage storageattach mate --storagectl IDE --port 1 --device 0 --type dvddrive --medium /iso/AlmaLinux-9.4-x86_64-boot.iso
...
vboxmanage controlvm mate keyboardputstring " selinux=0 inst.selinux=0 inst.text inst.ks=https://lxpro.com/almalinux9-mate.ks"
...
vagrant package --base mate --output almalinux9-mate-$(date +%Y%m%d).box

vboxmanage export mate -o almalinux9-mate-$(date +%Y%m%d).ova --vsys 0 --product almalinux9-mate --version $(date +%Y%m%d) --description "AlmaLinux 9 Developer Workstation with MATE Desktop"
