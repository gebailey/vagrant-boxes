...
# General settings
vboxmanage modifyvm mate --description "Fedora 40 with MATE Desktop"
...
vboxmanage storageattach mate --storagectl IDE --port 1 --device 0 --type dvddrive --medium /iso/Fedora-Server-netinst-x86_64-40-1.14.iso
...
vboxmanage controlvm mate keyboardputstring " selinux=0 inst.selinux=0 inst.text inst.ks=https://lxpro.com/fedora40-mate.ks"
# Ctrl-X
...
vboxmanage export mate -o fedora40-mate-$(date +%Y%m%d).ova --vsys 0 --product fedora40-mate --version $(date +%Y%m%d) --description "Fedora 40 with MATE Desktop"
