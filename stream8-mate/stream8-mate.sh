...
# General settings
vboxmanage modifyvm mate --description "CentOS Stream 8 Developer Workstation with MATE Desktop"
...
vagrant package --base mate --output stream8-mate-$(date +%Y%m%d).box

vboxmanage export mate -o stream8-mate-$(date +%Y%m%d).ova --vsys 0 --product stream8-mate --version $(date +%Y%m%d) --description "CentOS Stream 8 Developer Workstation with MATE Desktop"
