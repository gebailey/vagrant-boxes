...
network --hostname=fedora --bootproto=dhcp --device=link --ipv6=auto --activate
...
wget -nv https://go.dev/dl/go1.25.4.linux-amd64.tar.gz
tar -C /usr/local -xzf go1.25.4.linux-amd64.tar.gz
rm -f go1.25.4.linux-amd64.tar.gz
echo 'export PATH=$PATH:/usr/local/go/bin' > /etc/profile.d/golang.sh

wget -nv https://github.com/restic/restic/releases/download/v0.18.1/restic_0.18.1_linux_amd64.bz2
bunzip2 restic_0.18.1_linux_amd64.bz2
mv restic_0.18.1_linux_amd64 /usr/local/bin/restic
chmod 755 /usr/local/bin/restic

wget -nv https://astral.sh/uv/install.sh -O /usr/local/bin/install-uv.sh
chmod 755 /usr/local/bin/install-uv.sh
