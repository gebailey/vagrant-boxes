...
# General settings
vboxmanage modifyvm mate --description "Fedora 43 with MATE Desktop"
vboxmanage modifyvm mate --memory 4096 --vram 16 --cpus 2 --rtcuseutc on
...
vboxmanage createhd --filename "mate.vdi" --size 61440

vboxmanage storageattach mate --storagectl IDE --port 1 --device 0 --type dvddrive --medium /iso/Fedora-Server-netinst-x86_64-43-1.6.iso
...
vboxmanage controlvm mate keyboardputstring " selinux=0 inst.selinux=0 inst.text inst.ks=https://lxpro.com/fedora43-mate.ks"
# Ctrl-X
...
vboxmanage export mate -o fedora43-mate-$(date +%Y%m%d).ova --vsys 0 --product fedora43-mate --version $(date +%Y%m%d) --description "Fedora 43 with MATE Desktop"
