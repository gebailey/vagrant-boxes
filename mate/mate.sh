...
# General settings
vboxmanage modifyvm mate --description "CentOS 7 Developer Workstation with MATE Desktop"
...
vboxmanage createhd --filename "mate.vdi" --size 16384

vboxmanage storageattach mate --storagectl IDE --port 1 --device 0 --type dvddrive --medium /iso/CentOS-7-x86_64-DVD-1908.iso
...
vboxmanage controlvm mate keyboardputstring " selinux=0 inst.text inst.ks=http://192.168.7.7/mate.ks"
...
vboxmanage export mate -o mate-$(date +%Y%m%d).ova --vsys 0 --product mate --version $(date +%Y%m%d) --description "CentOS 7 Developer Workstation with MATE Desktop"
