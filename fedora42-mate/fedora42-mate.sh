...
# General settings
vboxmanage modifyvm mate --description "Fedora 42 with MATE Desktop"
...
vboxmanage storageattach mate --storagectl IDE --port 1 --device 0 --type dvddrive --medium /iso/Fedora-Server-netinst-x86_64-42-1.1.iso
...
vboxmanage controlvm mate keyboardputstring " selinux=0 inst.selinux=0 inst.text inst.ks=https://lxpro.com/fedora42-mate.ks"
# Ctrl-X
...
vboxmanage export mate -o fedora42-mate-$(date +%Y%m%d).ova --vsys 0 --product fedora42-mate --version $(date +%Y%m%d) --description "Fedora 42 with MATE Desktop"
