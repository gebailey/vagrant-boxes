...
# General settings
vboxmanage modifyvm mate --description "Fedora 44 with MATE Desktop"
...
vboxmanage storageattach mate --storagectl IDE --port 1 --device 0 --type dvddrive --medium /iso/Fedora-Server-netinst-x86_64-44-1.7.iso
...
vboxmanage controlvm mate keyboardputstring " selinux=0 inst.selinux=0 inst.text inst.ks=https://lxpro.com/fedora44-mate.ks"
# Ctrl-X
...
vboxmanage export mate -o fedora44-mate-$(date +%Y%m%d).ova --vsys 0 --product fedora44-mate --version $(date +%Y%m%d) --description "Fedora 44 with MATE Desktop"
